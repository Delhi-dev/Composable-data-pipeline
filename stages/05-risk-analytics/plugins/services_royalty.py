"""Service-fee & royalty base erosion typology plugin (indicative)."""

ID = "services_royalty"
VERSION = 1

def required_cbcr_indicators():
    return ["MX-001", "MX-003"]

def required_local_indicators():
    return ["service_fee_out_to_risk_juris", "royalty_ebitda_breach", "rpt_concentration", "wht_treaty_strip"]
...
def evaluate(entity, cbcr_flags, local_indicators, settings):
    ...
    weights = settings.get("scoring_weights.services_royalty", {}) or {}
...
def reason_codes(result, settings):
    return [{"code": "RC-SVC-001", "use_case_id": ID,
