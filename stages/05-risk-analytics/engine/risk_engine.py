"""Stage 05 engine — Risk Analytics (CbCR macro + typology plugins) (indicative).
...
def run(context):
    ...
    print(f"[{STAGE}] rules={len(rules)} sources={SOURCES} fired={len(fired)}  -> next: 06 Cross-Match & Scoring")
    return context
