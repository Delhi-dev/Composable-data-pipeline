"""Stage 04 engine — Data-Quality Gates & Analytical Pool (indicative).
...
def run(context):
    ...
    print(f"[{STAGE}] rules={len(rules)} sources={SOURCES} fired={len(fired)}  -> next: 05 Risk Analytics")
    return context
