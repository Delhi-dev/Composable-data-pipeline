"""Stage 06 engine — Cross-Match & Scoring (indicative).
...
STAGE = "cross-match-scoring"
SOURCES = ["transfer_pricing_return", "outward_remittance", "corporate_return", "audited_financials"]
...
def run(context):
    ...
    for key, client in clients.items():
        for rec in client.get("related_txns"):
            for rule in rules:
    ...
    print(f"[{STAGE}] rules={len(rules)} sources={SOURCES} fired={len(fired)}  -> next: 07 Case Selection")
    return context
