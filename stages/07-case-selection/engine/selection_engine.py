"""Stage 07 engine — Case Selection & Controlled Consumption (indicative).
...
def run(context):
    ...
    for key, client in clients.items():
        for rec in client.get("audit-cases"):
            for rule in rules:
    ...
    print(f"[{STAGE}] rules={len(rules)} sources={SOURCES} fired={len(fired)}  -> next: Case management (external) + governance")
    return context
